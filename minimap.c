...
static void	update_player_pos(t_data *data, t_vec_i square)
{
	...
	// actual player pos
	temp.x0 = square.x * data->pos.x + (SCREEN_WIDTH - data->minimap.width) - 20;
	temp.y0 = square.y * data->pos.y + 20;
	temp.color = 0xFF0000;
	render_rect(&data->image, temp);
	// intermittent point to dir
	temp.x0 = square.x * (data->pos.x + (data->dir.x / 4)) + (SCREEN_WIDTH - data->minimap.width) - 20;
	temp.y0 = square.y * (data->pos.y + (data->dir.y / 4)) + 20;
	render_rect(&data->image, temp);
	// dir point
	temp.x0 = square.x * (data->pos.x + (data->dir.x / 2)) + (SCREEN_WIDTH - data->minimap.width) - 20;
		...
		while (data->map.data[map_pos.y][++map_pos.x] != MAP_END_OF_LINE)
		{
			if (data->map.info[map_pos.y][map_pos.x].visible == 1)
			{
				...
				if (data->map.data[map_pos.y][map_pos.x] == MAP_TYPE_WALL || \
					data->map.data[map_pos.y][map_pos.x] == MAP_TYPE_WALL_SPRITE)
...
void	update_map(t_data *data)
{
	...
	while (i.y <= end.y)
	{
		...
		while (i.x <= end.x)
		{
			if (i.x >= 0 && i.y >= 0 && i.x <= data->map.x && i.y <= data->map.y)
